...
def load_classifier():
    ...
    model = timm.create_model(
        "convnext_base.fb_in22k_ft_in1k", pretrained=False, num_classes=len(classes)
    )
...
def process_image(image_path, mask_gen, model, classes, conf_thresh=0.5, detector_max_side=None):
    img_pil = Image.open(image_path).convert("RGB")
    img_np = np.array(img_pil)
    h, w = img_np.shape[:2]
    sam_input, sam_scale = resize_for_sam(img_np, detector_max_side)
    sam_h, sam_w = sam_input.shape[:2]
    img_area = sam_h * sam_w
...
def draw_output(img_path, dets, out_dir):
    ...
    for i, d in enumerate(dets):
        ...
        cv2.putText(
            img, txt, (x1 + 2, y1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2
        )
...
def main():
    ...
    if low_vram and args.points_per_side == 16:
        args.points_per_side = 12
    ...
    sam = sam_model_registry["vit_b"](checkpoint=SAM_CKPT).to(sam_device)
    ...
    print(
        f"[INFO] points_per_side={args.points_per_side} detector_max_side={args.detector_max_side} low_vram={low_vram}"
    )
    ...
    for impath in images:
        ...
        with open(txt_path, "w") as f:
            for d in dets:
                f.write(
                    f"{d['label']} {d['conf']:.4f} {d['box'][0]} {d['box'][1]} {d['box'][2]} {d['box'][3]}\n"
                )
